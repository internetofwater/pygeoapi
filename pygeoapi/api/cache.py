...
def headers_require_revalidation(request: 'APIRequest') -> bool:
    """Check if request headers indicate caching should be skipped. All header comparisons are done case insensitive"""
...
def lru_cache_specific_args(
    ...
    ----------
    key_func : Callable
        A function which takes the arguments that should be used as the key for the lru_cache and returns a tuple of the key arguments
    maxsize : int
        The maximum size of the cache
    skip_caching_fn : Callable[['APIRequest'], bool] | None
        An optional function which takes an APIRequest and returns a boolean indicating whether caching should be skipped for the given request
